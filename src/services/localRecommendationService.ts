...
class LocalRecommendationService {
  ...
  getSimilarProducts(productId: string, products: Product[], limit = 4): Product[] {
    ...
    const similarities = products
      .filter(p => p.id !== productId && (p.stock_quantity === undefined || p.stock_quantity === null || p.stock_quantity > 0) && p.is_active) // Filter out out-of-stock products
      .map(product => {
  ...
  getCartRecommendations(cartItems: CartItem[], products: Product[], limit = 4): Product[] {
    ...
    if (complementaryProducts.size > 0) {
      const recommended = Array.from(complementaryProducts)
        ...
        .filter(Boolean)
        .filter(p => (p.stock_quantity === undefined || p.stock_quantity === null || p.stock_quantity > 0) && p.is_active) // Filter out out-of-stock products
        .slice(0, limit)
  ...
  private getCategoryBasedRecommendations(
    ...
    const filtered = products
      .filter(product =>
        ...
        (product.stock_quantity === undefined || product.stock_quantity === null || product.stock_quantity > 0) && product.is_active // Filter out out-of-stock products
      )
  ...
  private getComplementaryCategories(categories: string[]): string[] {
    const rules: Record<string, string[]> = {
      ...
      'Напої': ['Закуски штучні', 'Сир', 'Закуски', 'Десерти'],
  ...
  getTrendingProducts(products: Product[], limit = 4): Product[] {
    const filtered = products
      .filter(product => (product.stock_quantity === undefined || product.stock_quantity === null || product.stock_quantity > 0) && product.is_active) // Filter out out-of-stock products
  ...
  getTimeBasedRecommendations(products: Product[], hour: number, limit = 4): Product[] {
    ...
    const filtered = products
      .filter(product => timeCategories.includes(product.category_name) && (product.stock_quantity === undefined || product.stock_quantity === null || product.stock_quantity > 0) && product.is_active)
      .sort((a, b) => (b.popularity_score || 0) - (a.popularity_score || 0))

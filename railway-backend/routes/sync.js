...
router.post('/inventory', async (req, res) => {
  ...
  try {
    ...
    syncLog = await createSyncLog('inventory', 'started')
    ...
    if (syncLog) {
      await updateSyncLog(syncLog.id, 'completed', totalInventoryRecords, null, JSON.stringify(result.stats))
    }

    console.log(`✅ Inventory sync completed! Updated ${totalInventoryRecords} records`)
    res.json(result)
    ...
    if (syncLog) {
      await updateSyncLog(syncLog.id, 'failed', 0, error.message)
    }
